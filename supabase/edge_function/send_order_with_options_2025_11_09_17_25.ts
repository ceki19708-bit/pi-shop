...
Deno.serve(async (req) => {
  ...
  try {
    const { productId, customerEmail, customerName, passphrase, selectedColor, selectedStorage } = await req.json()

    if (!productId || !customerEmail || !customerName || !passphrase) {
      return new Response(
    ...
    let configDetails = ''
    if (selectedColor) {
      configDetails += `🎨 Color: ${selectedColor}\n`
    }
    if (selectedStorage) {
      const storageLabel = product.category === 'smartwatch' ? 'Size' : 'Storage'
      configDetails += `💾 ${storageLabel}: ${selectedStorage}\n`
    }
